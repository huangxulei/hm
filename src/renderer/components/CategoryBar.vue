...
const { updateCurrentCategoryItem } = useSquareViewStore();
const { toggleCategoryView } = useMainViewStore();
...
<template>
    <div class="category-bar">
        <svg width="15" height="15" viewBox="0 0 29.3 29.3" @click.stop="toggleCategory">
        ...
        <template v-for="(cate, row) in data" v-show="data.length > 0">
            <span v-for="(item, col) in cate.data" @click="visitCateItem(item, row, col)" :class="{
                active:
                    row == currentCategoryItem.row &&
